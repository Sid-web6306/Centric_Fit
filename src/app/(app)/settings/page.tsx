...
const SettingsPage = () => {
  ...
  const tabs = [
    ...
    { id: 'gym' as const, label: 'Settings', icon: Building2, guard: 'gym' },
          ...
          <nav className="space-y-2">
            {tabs.map((tab) => (
              <button
                ...
                className={`flex items-center gap-3 px-4 py-3 rounded-lg text-left transition-colors cursor-pointer w-full lg:w-[220px] ${selectedTab === tab.id
        ...
        <div className="flex-1">
          ...
          {deferredTab === 'gym' && (
            <GymSettingsGuard action="update" fallback={
              <div className="p-8">
                <AccessDenied
                  message="Settings can only be modified by owners. Contact your owner if you need to make changes."
                />
              </div>
            }>
              <LazyGymTab />
...
const LazyGymTab = dynamic(
  () => import('@/components/settings/GymTab').then((m) => m.GymTab),
  {
    ssr: false,
